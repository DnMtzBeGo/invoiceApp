...
export class Step2Component implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (
      ...
      this.step2Form.get('unitType')!.setValue(changes.draftData.currentValue.cargo['53_48']);
      this.quantityunits = changes.draftData.currentValue.cargo.required_units;
      this.step2Form.get('cargoUnits')!.setValue(changes.draftData.currentValue.cargo.required_units);
      if (changes.draftData.currentValue.cargo.weigth) {
        this.step2Form.get('cargoWeight')!.setValue(changes.draftData.currentValue.cargo.weigth);
        this.editWeight = true;
      }
      
      this.step2Form.get('description')!.setValue(changes.draftData.currentValue.cargo.description);
    }
    if (changes.creationTime && changes.creationTime.currentValue) {
      ...
      if (this.draftDate > 0 && moment(this.calendar).valueOf() > this.draftDate) {
        ...
        this.events = moment(new Date(this.draftDate), 'MM-DD-YYYY').format('MMMM DD YYYY');
        this.step2Form.get('timepickup')!.setValue(new Date(this.draftDate));
      } else {
        this.step2Form.get('datepickup')!.setValue(this.calendar);
        this.events = moment(new Date(moment(this.calendar).valueOf()), 'MM-DD-YYYY').format('MMMM DD YYYY');
  ...
  handleCargoTypeChange(): void {
    ...
    if ( cargoType === 'hazardous') {
      ...
      this.step2Form.addControl('hazardousType', new FormControl(this.hazardousType, validators));
      this.step2Form.addControl('hazardousUn', new FormControl('', validators));
      this.step2Form.addControl('hazardousFile', new FormControl(this.hazardousFile, validators));
    } else {
