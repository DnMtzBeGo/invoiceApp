...
export class Step4Component implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.draftData && changes.draftData.currentValue && changes.draftData.currentValue.dropoff) {
      if(changes.draftData.currentValue.dropoff.startDate > 0 && changes.draftData.currentValue.dropoff.endDate > 0) {
        ...
        this.step4Form.get('datedropoff')!.setValue(new Date(changes.draftData.currentValue.dropoff.startDate));
        this.events = moment(changes.draftData.currentValue.dropoff.startDate).format('MMMM DD YYYY');        this.step4Form.get('extraNotes')!.setValue(changes.draftData.currentValue.dropoff.extra_notes);
      } else {
  ...
  timepickerValidStart (data: any) {
    this.lastTimeStart = this.step4Form.controls['timestartdropoff'].value || this.lastTimeStart;
  ...
  timepickerValidEnd (data: any) {
    this.lastTimeEnd = this.step4Form.controls['timeenddropoff'].value || this.lastTimeEnd;
